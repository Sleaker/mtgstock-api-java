...
public class PriceService extends AbstractMTGStockService {
	...
	private Prices getPricesFor(Integer id) throws IOException {
		...
		for (PRICES price : new PRICES[] { PRICES.LOW, PRICES.HIGH, PRICES.AVG, PRICES.MARKET, PRICES.MARKET_FOIL,
				PRICES.FOIL })
	...
	public SealedPricesAnalysis getSealedPrices(int id) {
		...
		log.debug("getting getSealedPrices at " + url);
		try {
			var obj = client.extractJson(url).getAsJsonObject();
			obj.entrySet().forEach(entry -> {

				var k = PRICES
						.valueOf(entry.getKey().equalsIgnoreCase("average") ? "AVG" : entry.getKey().toUpperCase());
				var pv = new PriceVariations(k);

				obj.get(entry.getKey()).getAsJsonArray()
						.forEach(e -> pv.put(Tools.initDate(e.getAsJsonArray().get(0).getAsLong()),
	...
	public SetPricesAnalysis getSetPricesAnalysis(Integer i) {
		...
		try {
			...
			for (MTGStockConstants.RARITY r : MTGStockConstants.RARITY.values())
				prices.getPriceHash().add(
						parsePriceHashFor(obj.get(PRICE_HASH).getAsJsonObject().get(r.name()).getAsJsonObject(), r));
			...
			if (obj.get(BOOSTER) != null) {
				...
				obooster.keySet().forEach(k -> phbooster.getAvg()
						.add(new EntryValue<>(PRICES.valueOf(k.toUpperCase()), obooster.get(k).getAsDouble())));
			}
			prices.setBooster(phbooster);

		} catch (Exception e) {
			log.error("Error getting SetPricesAnalysis for " + url, e);
		}
