...
public class AnalyticsService extends AbstractMTGStockService {
	...
	public List<HandyList> listHandyList() {
		...
		log.debug("getting HandyList at " + url);
		...
		try {
			...
			log.error("Error Getting handyList at " + url);
		}
	...
	public List<Print> listPrintsForHandyList(Integer hl) {
		...
		log.debug("listing HandyList prints at " + url);
	...
	public List<LowHighValues> listAllTimes() {
		...
		log.debug("getting low/high value at " + url);
		try {
			...
			for (JsonElement el : arr) {
				...
				val.setPrice(new EntryValue<>(
						Tools.initDate(obj.get(PRICE).getAsJsonObject().get(DATE).getAsString(), DATE_FORMAT),
						obj.get(PRICE).getAsJsonObject().get(AVG).getAsDouble()));
				ret.add(val);
			}

		} catch (IOException e) {
			log.error("Error getting low/hig values", e);
		}
	...
	public SetPrices getExpectedValue(CardSet s) {
		Optional<SetPrices> opt = getExpectedValues().stream().filter(sp -> sp.getSet().getName().equals(s.getName()))
				.findAny();
	...
	public List<SetPrices> getExpectedValues() {
		...
		try {
			...
			for (JsonElement el : arr) {
				...
				prices.keySet().stream().filter(s -> !s.equalsIgnoreCase("num"))
						.forEach(key -> p.put(PRICES.valueOf(key.toUpperCase()), prices.get(key).getAsDouble()));
			...
			log.error("error getting expected value at " + url, e);
		}
	...
	public List<Played> getMostPlayedCard(FORMAT f) {
		...
		log.debug("get MostPlayedCard to " + url);
		try {
			...
			arr.forEach(k -> {
				...
				p.setId(k.getAsJsonObject().get(CARD).getAsJsonObject().get(PRINT).getAsJsonObject().get(ID)
						.getAsInt());

				if (k.getAsJsonObject().get(CARD).getAsJsonObject().get(PRINT).getAsJsonObject().get(IMAGE) != null)
					p.setImage(k.getAsJsonObject().get(CARD).getAsJsonObject().get(PRINT).getAsJsonObject().get(IMAGE)
							.getAsString());

				p.setQuantity(k.getAsJsonObject().get(QUANTITY).getAsInt());

				if (!k.getAsJsonObject().get(CARD).getAsJsonObject().get(PRINT).getAsJsonObject().get(LATEST_PRICE)
						.getAsJsonObject().get(AVG).isJsonNull())
					p.setAvgPrice(k.getAsJsonObject().get(CARD).getAsJsonObject().get(PRINT).getAsJsonObject()
							.get(LATEST_PRICE).getAsJsonObject().get(AVG).getAsDouble());
				ret.add(p);
			});
		} catch (IOException e) {
			log.error("Error getting mostplayedCard at " + url + " : " + e);
		}
		return ret;
	}
	...
	public List<Metagame> getMetagamesFor(FORMAT f) {
		...
		try {
			...
			log.error("Error getting metagames analytics for " + f, e);
		}
