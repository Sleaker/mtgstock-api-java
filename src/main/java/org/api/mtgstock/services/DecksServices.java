...
public class DecksServices extends AbstractMTGStockService {
	...
	public List<DeckInfo> listDeckForTournament(Integer id) {
		...
		try {
			...
			log.error("error getting decks for tournament=" + id, e);
		}
	...
	public Deck getDecksDetails(Integer id) {
		...
		try {
			...
			obj.get(MAINBOARD).getAsJsonArray()
					.forEach(je -> d.getMainboard().put(
							parseDeckCardFor(je.getAsJsonObject().get(CARD).getAsJsonObject()),
							je.getAsJsonObject().get(QUANTITY).getAsInt()));
			obj.get(SIDEBOARD).getAsJsonArray()
					.forEach(je -> d.getSideboard().put(
							parseDeckCardFor(je.getAsJsonObject().get(CARD).getAsJsonObject()),
							je.getAsJsonObject().get(QUANTITY).getAsInt()));
		} catch (Exception e) {
			log.error(e);
	...
	public List<Tournament> listTournaments(FORMAT f) {
		...
		log.debug("list tournament at " + url);
		try {
			...
			log.error("error getting tournaments for " + f, e);
		}
	...
	public List<Archetype> listArchetypes(FORMAT f) {
		...
		log.debug("list archetypes at " + url);
		...
		try {
			...
			log.error("error getting archetypes for " + f, e);
		}
		return ret;
	}
